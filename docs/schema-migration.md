...
- `schema_version = 4`
  - Adds optional registry-local account groups for the legacy `group use` scope
  - Adds optional `active_group` to scope plain `list`, `switch`, and background candidate selection inside one `CODEX_HOME`
