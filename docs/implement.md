...
- The staged preview root package also gets a `codexAuthPreviewLabel` field like `pr-6 b6bfcf5`; the root CLI wrapper uses that field so `codex-auth --version` prints `codex-auth 0.1.5 (preview pr-6 b6bfcf5)` for preview installs only.
