...
const SyncBddContext = struct {
    ...
    fn givenRegisteredAccount(self: *SyncBddContext, email: []const u8, alias: []const u8, plan: registry.PlanType) !void {
        try bdd.appendChatgptAccount(self.allocator, &self.reg, email, alias, plan);
    }
...
test "Scenario: Given empty registry when syncing auth then auto import and activate" {
    ...
    try std.testing.expect(ctx.reg.accounts.items.len == 1);
    try std.testing.expect(ctx.reg.active_account_id != null);
}

test "Scenario: Given unmatched active auth plan when syncing then append account and switch active" {
    const gpa = std.testing.allocator;
    var ctx = try SyncBddContext.givenCleanCodexHome(gpa);
    defer ctx.deinit();

    try ctx.givenRegisteredAccount("user@example.com", "old", .free);
    try registry.setActiveAccount(gpa, &ctx.reg, ctx.reg.accounts.items[0].account_id);

    const active_auth = try bdd.authJsonWithEmailPlan(gpa, "user@example.com", "team");
    defer gpa.free(active_auth);
    try ctx.givenActiveAuthJson(active_auth);
    ...
    try std.testing.expect(changed);
    try std.testing.expect(ctx.reg.accounts.items.len == 2);
    try std.testing.expect(ctx.reg.active_account_id != null);
}
...
test "Scenario: Given accounts with different remaining usage when selecting best then highest remaining wins" {
    ...
    try bdd.appendChatgptAccount(gpa, &reg, "low@example.com", "", .pro);
    try bdd.appendChatgptAccount(gpa, &reg, "high@example.com", "", .team);
